...
parser.add_option("--input", dest="input", default='MetTree_25ns_1560pb.root', type="string", action="store", help="input file.Default:MetTree_25ns_1560pb.root")
...
l = ROOT.TLegend(0.19 ,0.8, .57, .95)
l.AddEntry(px, "< #slash{E}_{x} >"+("  #chi^{2} / nDOF = %.2f / %.2f" % (fx.GetChisquare() ,  fx.GetNDF())) ) # +etab[0].split('_')[0]+", "+shortName[etab[0].split('_')[1]])
l.AddEntry(py, "< #slash{E}_{y} >"+("  #chi^{2} / nDOF = %.2f / %.2f" % (fy.GetChisquare() ,  fy.GetNDF())) )
#l.AddEntry(fx, ("#chi^{2} / nDOF = %.2f / %.2f" % (fx.GetChisquare() ,  fx.GetNDF())) ) 
#l.AddEntry(fy, ("#chi^{2} / nDOF = %.2f / %.2f" % (fy.GetChisquare() ,  fy.GetNDF())) )
#l.AddEntry(fx, ("(#chi)^2 %.2f / nDOF %.2f" % (fx.GetChisquare() ,  fx.GetNDF())) ) 
#l.AddEntry(fx, "nDOF %.2f" % fx.GetNDF())
#l.AddEntry(fy, "nDOF %.2f" % fy.GetNDF())
l.SetFillColor(0)

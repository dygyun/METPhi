...
//      }
    ...
    //     std::cout<<"cand pdgId "<<c.pdgId()<<" testing type:"<<type_[j]<<" translated to pdg:"<<translateTypeToAbsPdgId(reco::PFCandidate::ParticleType(type_[j])) <<std::endl;
        }  ///////////min Eta max range..... metx, mety
      
      }//////pdgId  metx, mety
    }  ////////particle type like h, h0....
//////////////////////////////////////////////////////////////
...
//    std::cout<<"j "<<j<<" "<<v->getParameter<std::string>("name")<<" varType "<<varType_[j]<<" counts "<<counts_[j]<<" sumPt "<<sumPt_[j]<<" nvtx "<<ngoodVertices<<" "<<MEx_[j]<<" "<<MEy_[j]<<std::endl;
    if (varType_[j]==0) {
      METx_[j]->Fill(MEx_[j]);
      METy_[j]->Fill(MEy_[j]);
      double metPhi = (TMath::ATan2(MEy_[j],MEx_[j]) );
      METphi_[j]->Fill(metPhi);
      profile_x_[j]->Fill(counts_[j], MEx_[j]);//Profile of MEx vs PF candidate multiplicity
      profile_y_[j]->Fill(counts_[j], MEy_[j]);
      variable_[j]->Fill(counts_[j]);
    } 
    //std::cout<<" METx "<< METx_[j] <<"----MEtPhi "<< METphi_[j]  <<"______eta "<< c.eta() <<std::endl;
    if (varType_[j]==1) {
